...
public class Solution {
    ...
    public static class Gamer extends Thread {
        ...
        public void run() {
            ...
            int num = 0; //начало игры
            while (!OnlineGame.isWinnerFound && num < OnlineGame.steps.size()) //проверка победителя и оставшихся шагов игры
            {
                System.out.println(getName() + ":" + OnlineGame.steps.get(num));
                try
                {
                    Thread.sleep(1000 / rating); //учитываем скорость выполнения шагов
                }
                catch (InterruptedException e)
                {
                    System.out.println(getName() + ":проиграл"); //кто не успел, тот проиграл
                }
                num++;
            }
            //действия выполнены, игра закончена
            if (num == OnlineGame.steps.size()) {
                OnlineGame.isWinnerFound = true;
                System.out.println(getName() + ":победитель!"); //определение победителя
            }
        }
    }
}
